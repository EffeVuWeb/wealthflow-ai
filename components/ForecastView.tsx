...
import { LineChart, TrendingUp, TrendingDown, AlertCircle, Sparkles, Loader2 } from './Icons';
...
const ForecastView: React.FC<ForecastViewProps> = ({ transactions, subscriptions, loans, accounts }) => {
    ...
    React.useEffect(() => {
        const fetchForecast = async () => {
            ...
            const insight = await generateForecast(
                forecastData.avgMonthlyIncome,
                forecastData.avgMonthlyVarExpense + forecastData.totalMonthlyFixed,
                forecastData.data[0].balance,
                ((forecastData.netMonthlyFlow / forecastData.avgMonthlyIncome) * 100) || 0
            );
            setAiInsight(insight);
            setLoadingInsight(false);
        };

        if (forecastData.avgMonthlyIncome > 0) {
            fetchForecast();
        }
    }, [forecastData]);
        ...
        <div className="space-y-6">
            <div className="bg-gradient-to-r from-indigo-900/40 to-slate-900 border border-indigo-500/30 rounded-2xl p-6">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    {/* AI Insight Card */}
                    <div className="mb-6 bg-gradient-to-r from-violet-600/20 to-fuchsia-600/20 border border-violet-500/30 rounded-xl p-4 flex items-start gap-3">
                        <div className="bg-violet-500/20 p-2 rounded-lg">
                            {loadingInsight ? <Loader2 className="w-5 h-5 text-violet-400 animate-spin" /> : <Sparkles className="w-5 h-5 text-violet-400" />}
                        </div>
                        <div>
                            <h3 className="text-violet-200 font-bold text-sm mb-1">AI Smart Forecast</h3>
                            <p className="text-slate-300 text-sm leading-relaxed">
                                {loadingInsight ? "Analisi dei trend futuri in corso..." : (aiInsight || "Dati insufficienti per una previsione.")}
                            </p>
                        </div>
                    </div>
                </div>
